...
import numpy as np
import os
filename = os.environ.get('Sigmoid.py')
...
class RNA(object):
    def __init__(self, amostras, saida_desejada,taxa_aprendizado = 0.6,max_iteracoes = 100):
        ...
        self.N_saidas_desejada       = len(self.saida_desejada[0])
        #saida camadas
        self.y_hidden               = np.zeros((1,self.N_neuronios_hidden))
        self.y_output               = np.zeros((1,self.N_saidas_desejada))
        #pesos
        self.w1                     = np.zeros((self.N_neuronios_hidden,self.N_entradas_amostra))
        self.w2                     = np.zeros((self.N_saidas_desejada,self.N_neuronios_hidden))
        self.bias_hidden            = np.zeros((1,self.N_neuronios_hidden))
        self.bias_output            = np.zeros((1,self.N_saidas_desejada))
        self.media_erros            = [] 
    ...
    def treinar(self):
        ...
        #iniciando os valores do bias_output
        for i in range(self.N_saidas_desejada):
    ...
    def classificar(self,entradas):
        ...
        for i in range(self.N_neuronios_hidden):
            ...
            for j in range(N_entradas):
                self.y_hidden[i] = self.y_hidden[i]+self.w1[i][j]*entradas[j]
            #gerando saída na função sigmoid
            self.y_hidden[i] = func_sigmoid(self.y_hidden[i]-self.bias_hidden[i])
        
        
        #passar pela camada de saida
        for i in range(len(self.N_saidas_desejada)):
            self.y_output[i] = 0
            for j in range(self.N_neuronios_hidden):
                self.y_output[i] = self.y_output[i] + self.w2[i][j]*self.y_hidden[j]
            #gerando saída na função sigmoid
...
mlp_teste = RNA([
    [5.4143541026860476,-1.3560495339334011],
    [5.4143541026860476,-1.3560495339334011],
    [5.4143541026860476,-1.3560495339334011],
    [5.4143541026860476,-1.3560495339334011],
    [5.4143541026860476,-1.3560495339334011],
